...
export function Combobox({
  ...
  const getCommandContent = (isInDrawer: boolean = false) => (
    <Command className={cn(
      ...
      <CommandList className={cn(
        ...
        <CommandGroup className={isInDrawer ? "shadow-none border-none" : ""}>
          {options.map((option) => (
            <CommandItem
              key={option.value}
              value={option.value}
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] max-h-[300px] p-0 bg-white dark:bg-[#1E1F25] border-gray-200 dark:border-gray-700" align="start">
